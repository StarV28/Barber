...
***************************************************************************** */
        ...
        function pageActive(page) {
            let reguljar = /\/([^\/]+)\.html/;
            let match = reguljar.exec(page);
            if (!match) match = [ "", "index" ];
            delateTestimon(match[1]);
            let menuLinkArr = document.querySelectorAll(".menu__item");
            menuLinkArr.forEach((link => {
                if (link.getAttribute("id") === match[1]) link.classList.add("active");
            }));
        }
        function delateTestimon(page) {
            let testimonual = document.getElementById("testimon");
            if (page != "index") testimonual.style.display = "none";
        }
        window.onload = e => {
